...
// Note (the record-carried annotation value) lives in `shared/note` — its OWN module,
// opened only by note-carrying consumers, per the inert-value scope rule (2026-08-10):
// a seat here would ride EVERY domain cone at default scope for pure solve-time cost.
