...
/*
 * Static-model tests for InventoryItem (v1). Illustrate decisions D1–D14: the stored/derived
 * split, the non-negative cone, the worthiness derivation (ENABLED/DEGRADED/DISABLED from
 * degradedQty), Fill, EMPTY⟹ENABLED, LPN/serial uniqueness, commingled lots, and the forbidden
 * (negative / EMPTY×degraded / degraded>actual / serial-dup) states.
 *
 * `unit_*` runs expecting SAT illustrate reachable states; `check`s (UNSAT = holds) prove
 * invariants; the trailing negative `run`s expect UNSAT (the design forbids them).
 */
...
run unit_inventoryItem_locked {
  some ii: InventoryItem | ii.administrativeState = LOCKED and not isZero[ii.actualQuantity.byUnit]
} for 5 but 3 Scalar

// FULL: actual resolvably-equals maxQty.
run unit_inventoryItem_full { some ii: InventoryItem | ii.fillState = FULL } for 5 but 3 Scalar

// Commingled lots: an item holding ≥ 2 lot numbers (D11).
run unit_inventoryItem_commingledLots { some ii: InventoryItem | gt[#ii.lotNumbers, 1] } for 5 but 3 Scalar

// Serialized item (D10).
run unit_inventoryItem_serialized { some ii: InventoryItem | isSerialized[ii] } for 5 but 3 Scalar
...
run unit_inventoryItem_descriptive {
  some ii: InventoryItem | some ii.notes and some ii.colorCode
} for 5 but 3 Scalar
...
assert unit_inventoryItem_availableNonNegative {
  all ii: InventoryItem | classify[ii.availableQty] in (ZERO + POSITIVE)
}
...
// License plates are unique across items (D9).
...
// ── negative scenarios (expect UNSAT — the design forbids them) ───────────────────────

// No negative stored quantity (the cone).
run unit_inventoryItem_negActualImpossible {
  some ii: InventoryItem | classify[ii.actualQuantity.byUnit] = NEGATIVE
} for 5 but 3 Scalar

// No degradedQty on an EMPTY item.
run unit_inventoryItem_emptyDegradedImpossible {
  some ii: InventoryItem | isZero[ii.actualQuantity.byUnit] and some ii.degradedQty
} for 5 but 3 Scalar

// No degradedQty exceeding actual.
run unit_inventoryItem_degradedAboveActualImpossible {
  some ii: InventoryItem | some ii.degradedQty and not lte[ii.degradedQty.byUnit, ii.actualQuantity.byUnit]
} for 5 but 3 Scalar
